...
for (const width of [1200, 968, 640, 390]) {
  test(`tag editor stays separate from the grid at ${width}px and follows row selection`, async ({ page }, testInfo) => {
    ...
    expect(await first.locator('xpath=ancestor::tr').evaluate(row => row.getBoundingClientRect().height)).toBe(24);
    ...
    expect(selectionColor.actual).not.toBe('rgba(0, 0, 0, 0)');
    const layout = await page.evaluate(() => {
      ...
      return {
        ...
        sidebarSeparate: sidebar.right <= g.left && sidebar.right <= p.left,
        gridHeight: g.height,
    ...
    expect(layout.sidebarSeparate).toBe(true);
    ...
    await page.screenshot({ path: testInfo.outputPath(`restored-tags-forest-palace-${width}.png`), fullPage: true, animations: 'disabled' });
...
test('tag corrections are reachable from the track context menu without changing selection or playback', async ({ page }, testInfo) => {
  await backend(page, false);
  await page.route('**/api/items', route => route.fulfill({ json: tracks.map((track, index) => ({ ...track,
    bookmarks: index ? {} : { moment: { position: 90, label: 'Piano entrance', emoji: '🎹', created_time_utc: '2026-09-01T12:00:00' } },
  })) }));
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.addInitScript(() => localStorage.setItem('reitunes-theme', JSON.stringify({ lightTheme: 'neutral', darkTheme: 'forest-palace', mode: 'dark' })));
  await page.goto('/');
  const row = page.getByRole('row').filter({ hasText: 'Evening set' });
  await row.locator('[data-column=name]').click({ button: 'right' });
...
test('selected track generation shows phases, survives closing the editor, and offers retry on failure', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Close tags' }).click();
  await expect(page.locator('.source-sidebar').getByRole('button', { name: 'Tags', exact: true })).toHaveAttribute('title', '1 tracks getting tags');
  await page.locator('.source-sidebar').getByRole('button', { name: 'Tags', exact: true }).click();
