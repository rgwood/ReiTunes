...
public class SQLiteEventRepository : ISerializedEventRepository
{
    ...
    public IEnumerable<IEvent> GetAllEvents()
        => GetAllSerializedEvents().Select(s => EventSerialization.Deserialize(s));

    public IEnumerable<string> GetAllSerializedEvents()
        => GetSerializedEvents("select Serialized from events;");

    private List<string> GetSerializedEvents(string sql, object param = null)
        => _conn.Query<string>(sql, param).ToList();

    public IEnumerable<IEvent> GetEvents(Guid aggregateId)
    {
        List<string> serializedEvents = GetSerializedEvents("select Serialized from events WHERE AggregateId = @AggregateId",
    ...
    public int CountOfAllEvents()
        => _conn.QuerySingle<int>($"SELECT COUNT() FROM events;");
}
